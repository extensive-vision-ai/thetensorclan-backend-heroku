...
def classify_resnet34_imagenet(model: RecursiveScriptModule, classes: List[str], image: Image.Image) \
        -> List[Dict[str, Any]]:
...
def classify_mobilenetv2_ifo(model: RecursiveScriptModule, classes: List[str], image: Image.Image) \
        -> List[Dict[str, Any]]:
    trans: Compose = T.Compose([
        T.Resize(256),
        T.CenterCrop(224),
        T.ToTensor(),
        T.Normalize(mean=[0.533459901809692, 0.584880530834198, 0.615305066108704],
...
def classify_indian_face(model: RecursiveScriptModule, classes: List[str], image: Image.Image) \
        -> List[Dict[str, Any]]:
    """
...
def classify_lfw_plus(model: RecursiveScriptModule, classes: List[str], image: Image.Image) -> List[Dict[str, Any]]:
    """
    The image sent to this MUST be aligned
    """
    trans: Compose = T.Compose([
